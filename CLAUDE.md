...
### Планируется
- [ ] **Мультиязычные версии документов** - связь узбекских, русских и английских версий одного документа
  - Добавить поле `document_group_id` в модель Document
  - Узбекская версия (главная) имеет `document_group_id = свой id`
  - Русская/английская версии имеют `document_group_id = id узбекской версии`
  - API возвращает список доступных языковых версий
  - Пример: `GET /api/documents/1` → `{..., versions: [{language: "ru", file_path: "..."}, {language: "en", ...}]}`
